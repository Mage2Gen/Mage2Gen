	...
	def add(self, api_name, api_method='GET',extra_params=None):
		...
		management_interface.add_method(
			InterfaceMethod(
				...
				params=['$param'],
				docstring=[description, '\n', '@param string $param', '@return string']
			)
		)
		...
		model.add_method(Phpmethod(
			...
			body="return 'Api " + api_method + " method, which returns it\\'s parameter as example: \' . $param;"
		))
				...
				Xmlnode(
					...
					nodes=[
						Xmlnode(
							...
							attributes={
								'class': api_classname,
								'method': api_method.lower() + upperfirst(api_name)
							}
						),
